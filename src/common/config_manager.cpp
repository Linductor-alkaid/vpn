...
std::vector<ConfigManagerInterface::VPNConnectionProfile> ConfigManager::loadAllProfiles() {
    std::lock_guard<std::mutex> lock(config_mutex_);
    std::vector<VPNConnectionProfile> profiles;
    
    try {
        auto config_files = listConfigFiles();
        
        for (const auto& filename : config_files) {
            std::string file_path = config_dir_ + PATH_SEPARATOR + filename;
...
std::unique_ptr<ConfigManagerInterface::VPNConnectionProfile> ConfigManager::loadProfile(const std::string& name) {
    std::lock_guard<std::mutex> lock(config_mutex_);
...
std::vector<ConfigManagerInterface::VPNConnectionProfile> ConfigManager::getRecentProfiles(size_t count) {
...
std::vector<ConfigManagerInterface::VPNConnectionProfile> ConfigManager::getFavoriteProfiles() {
    auto all_profiles = loadAllProfiles();
...
std::unique_ptr<ConfigManagerInterface::VPNConnectionProfile> ConfigManager::getAutoConnectProfile() {
    auto all_profiles = loadAllProfiles();
...
std::string ConfigManager::getConfigFilePath(const std::string& name) {
    return config_dir_ + PATH_SEPARATOR + sanitizeFileName(name) + ".json";
}
...
std::string ConfigManager::getConfigDirectory() {
    ...
    return std::string(home) + "/.config/sduvpn";
#endif
}
...
#else
    return mkdir(config_dir_.c_str(), 0755) == 0 || errno == EEXIST;
#endif
}
...
bool ConfigManager::deleteFile(const std::string& path) {
#ifdef _WIN32
...
std::vector<std::string> ConfigManager::listConfigFiles() {
...
#else
    DIR* dir = opendir(config_dir_.c_str());
    if (dir) {
        struct dirent* entry;
...
std::string ConfigManager::profileToJson(const VPNConnectionProfile& profile) {
    ...
    json << "  \"interface_name\": \"" << profile.interface_name << "\",\n";
...
std::unique_ptr<ConfigManagerInterface::VPNConnectionProfile> ConfigManager::profileFromJson(const std::string& json) {
    ...
    try {
        ...
        profile->interface_name = getValue("interface_name");
...
std::unique_ptr<ConfigManagerInterface::VPNConnectionProfile> ConfigManager::findProfileByLoginData(
...
bool ConfigManager::importConfig(const std::string& file_path) {
    try {
        std::ifstream file(file_path);
        if (!file.is_open()) {
            return false;
        ...
        // 生成唯一名称避免冲突
        profile->name = generateUniqueName(profile->name);
...
bool ConfigManager::exportConfig(const std::string& file_path, const std::vector<std::string>& profile_names) {
    try {
        std::ofstream file(file_path);
        if (!file.is_open()) {
            return false;
        ...
        for (size_t i = 0; i < profile_names.size(); ++i) {
            auto profile = loadProfile(profile_names[i]);
            if (profile) {
                if (i > 0) {
                    file << ",\n";
                }
